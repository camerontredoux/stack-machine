...
/*    test("load test") {
        val (newstack, newenv) = StackMachineEmulator.emulateSingleInstruction(newstack, newenv, LoadIns("x"))
    ...
    test("list of loads") {
        val loadList = List(PushIns(4), LoadIns("bar"))
        val f = StackMachineEmulator.emulateStackMachine(loadList)
        testValueOfIdent(f, "bar", 4.0)
    }
