...
class CompilerTest extends FunSuite {
    ...
    test("nested add test") {
        val e1 = Plus(Const(1.0), Const(2.0))
        val e2 = Plus(Const(3.0), Const(4.0))
        val e = Plus(e1, e2)
        val lst = StackMachineCompiler.compileToStackMachineCode(e)
        //println("----")
        //lst.foreach(println)
        //println("----")
        assert(lst == List(PushIns(1.0), PushIns(2.0), AddIns, PushIns(3.0), PushIns(4.0), AddIns, AddIns))
    }

    test("ident") {
        val lst = StackMachineCompiler.compileToStackMachineCode(Ident("x"))
        assert(lst == List(StoreIns("x")))
    }

    test("let expression with ident") {
        val e = Let("x", Const(4.0), Ident("x"))
        val lst = StackMachineCompiler.compileToStackMachineCode(e)
        assert(lst == List(PushIns(4), LoadIns("x"), StoreIns("x")))
    }
